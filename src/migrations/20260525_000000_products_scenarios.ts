...
DO $$ BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'products_scenarios_locales'
  ) THEN
...
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'products_scenarios_locales'
...
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = '_products_v_version_scenarios_locales'
  ) THEN
...
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = '_products_v_version_scenarios_locales'
      AND column_name = '_parent_id'
      AND data_type <> 'character varying'
  ) THEN
