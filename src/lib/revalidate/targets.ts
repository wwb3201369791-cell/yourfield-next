...
const safePathSchema = z
  ...
  .trim()
  .max(512)
  .refine((value) => isSafeRevalidationPath(value), 'Path must be an internal localized public path.');
...
export async function buildRevalidationTargets(
  ...
  if (pathSet.size === 0 && tagSet.size === 0) {
    skipped.push({ reason: 'no-safe-targets', target: input.collection ?? input.global ?? 'request' });
  }
