...
const searchClickBodySchema = z
  .object({
    ...
    result: z
      .object({
        id: z.preprocess(trimAndCollapse, safeSearchTextSchema(1, MAX_CLICK_RESULT_ID_LENGTH)),
        title: z.preprocess(
          trimAndCollapse,
        ...
        url: z.preprocess(
          trimAndCollapse,
          z.string().min(1).max(MAX_CLICK_RESULT_URL_LENGTH).refine(isSafeInternalSearchUrl, {
