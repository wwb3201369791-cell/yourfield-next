import { requiredProductI18nPaths, type RequiredProductI18nPath } from '../product/productI18nRequirements';
...
function specsFromRequiredPaths(paths: readonly RequiredProductI18nPath[]) {
  ...
  return paths.map((item): CheckSpec & { group: string; originalPath: string; section: ProductEditorSection } => {
    const hasChildSpec = Array.from(normalizedPaths).some((candidate) => candidate.startsWith(`${item.path}.`));
