  ...
  for (const field of fields) {
    ...
    if (field.type === 'tabs') {
      for (const tab of field.tabs) {
        const tabPath =
          'name' in tab && typeof tab.name === 'string' ? [...parentPath, tab.name] : parentPath;
        specs.push(
          ...collectCheckSpecs(tab.fields, tabPath, fieldLocalized, parentLabel, blockType),
        );
      }
      continue;
    }
    ...
    if (field.type === 'row' || field.type === 'collapsible') {
      specs.push(
        ...collectCheckSpecs(field.fields, parentPath, fieldLocalized, parentLabel, blockType),
      );
      continue;
    }
    ...
    if (field.type === 'group') {
      specs.push(
        ...collectCheckSpecs(field.fields, currentPath, fieldLocalized, currentLabel, blockType),
      );
      continue;
    }
...
export function docsByLocale(args: {
  ...
  for (const locale of args.locales) {
    const existingLocaleDoc = flattenDocumentForLocale(
      args.existingAllLocales,
      args.fields,
      locale,
    );
    const doc =
