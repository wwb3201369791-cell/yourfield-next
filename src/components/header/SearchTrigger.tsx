      ...
      {showHotTerms ? (
        <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-[1400] overflow-hidden rounded border border-white/70 bg-white/95 p-3 shadow-xl backdrop-blur-xl">
          ...
          <ul
            id={suggestionsId}
            role="listbox"
            aria-label={t('search.popular')}
            className="flex flex-wrap gap-2"
          >
            {visibleHotTerms.map((term) => (
              <li key={term} role="presentation">
                <button
                  className="border-line rounded-full border bg-white px-3 py-1.5 text-xs font-semibold text-primary transition hover:border-accent hover:text-accent focus:border-accent focus:text-accent focus:outline-none"
                  type="button"
                  role="option"
                  aria-selected={false}
