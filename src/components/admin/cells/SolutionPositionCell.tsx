'use client';

import { useAdminText } from '../adminUiLocale';

type CellProps = Readonly<{
  cellData?: unknown;
  data?: unknown;
  rowData?: Readonly<Record<string, unknown>>;
}>;

function numericOrder(value: unknown) {
  return typeof value === 'number' && Number.isFinite(value) ? value : null;
}

function positionLabel(order: number | null, t: ReturnType<typeof useAdminText>) {
  if (order === null || order <= 0) {
    return t('未设置');
  }

  return t({ en: `Position ${order}`, zh: `第 ${order} 位` });
}

export default function SolutionPositionCell({ cellData, data, rowData }: CellProps) {
  const t = useAdminText();
  const order = numericOrder(data) ?? numericOrder(cellData) ?? numericOrder(rowData?.order);

  return <span className="yf-position-badge">{positionLabel(order, t)}</span>;
}
