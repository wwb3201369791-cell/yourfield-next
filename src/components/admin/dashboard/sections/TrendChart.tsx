  ...
  return (
    <article className="yourfield-ops-panel yourfield-ops-panel--chart">
      <div className="yourfield-ops-panel__head">
        <div>
          <h3>{t({ en: `${rangeLabel} interaction trend`, zh: `${rangeLabel}互动趋势` })}</h3>
          <p>{t('按天展示搜索、点击与询盘的变化曲线。')}</p>
        </div>
        <div className="yourfield-ops-trend__legend" role="group" aria-label={t('切换显示的指标')}>
          {SERIES.map((series) => {
            const isHidden = hiddenSeries.includes(series.key);
            const chipClassName = isHidden
              ? 'yourfield-ops-trend__chip is-hidden'
              : 'yourfield-ops-trend__chip';
      ...
      <div className="yourfield-ops-trend">
        <svg
          ...
          role="img"
          aria-label={t({
            en: `${rangeLabel} interaction trend line chart`,
            zh: `${rangeLabel}互动趋势折线图`,
          })}
