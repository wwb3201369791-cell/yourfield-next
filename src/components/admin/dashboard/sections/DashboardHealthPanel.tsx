...
function healthSummary(health: DashboardHealthResponse, t: Translate) {
  const activeItems = health.items.filter((item) => item.count > 0);

  if (activeItems.length === 0) {
    return t('内容、询盘和新闻更新暂未发现明显问题。');
  }

  const severeCount = activeItems.filter((item) => item.severity === 'severe').length;
  if (severeCount > 0) {
    return t({
      en: `${severeCount} issue types need priority handling.`,
      zh: `有 ${severeCount} 类问题需要优先处理。`,
    });
  }

  return t({
    en: `${activeItems.length} content reminder types can be improved.`,
    zh: `有 ${activeItems.length} 类内容提醒可继续优化。`,
  });
}
...
function HealthRing({ health }: Readonly<{ health: DashboardHealthResponse }>) {
  ...
  return (
    <div
      ...
      aria-label={t({
        en: `Website content health score ${health.score}`,
        zh: `网站内容健康度 ${health.score} 分`,
      })}
    >
        ...
        <em>
          {isClear
            ? showClearState
              ...
              : ''
            : t({
                en: `${t(dashboardHealthSeverityLabel(item.severity))} · ${item.count} items`,
                zh: `${dashboardHealthSeverityLabel(item.severity)} · ${item.count} 项`,
              })}
        </em>
      </span>
      {isClear ? null : (
  ...
  return (
    ...
    >
      <div className="yourfield-ops-panel__head">
        <div>
          <h3>{isDetail ? t('网站内容健康度') : t('内容健康度')}</h3>
          <p>{healthSummary(health, t)}</p>
        </div>
        {isDetail ? (
          <span
          ...
          >
            ...
            {t(dashboardHealthLevelLabel(health.level))}
          </span>
        ) : (
          <a
      ...
      <div className="yourfield-ops-health__overview">
        ...
        <div className="yourfield-ops-health__stats" aria-label={t('健康度问题分类')}>
          <span>
      ...
      <div className="yourfield-ops-health-list">
        {displayItems.length > 0 ? (
          ...
          <p className="yourfield-ops-empty">{t('暂无需要处理的内容问题。')}</p>
        )}
      </div>
    </article>
  );
