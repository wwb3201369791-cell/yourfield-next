...
export function SizeGuideDrawer() {
  ...
  const applySampleGuide = () => {
    setTitle(t('尺码对应表'));
    setCornerLabel(t({ en: 'Height cm / Weight kg', zh: '身高 cm / 体重 kg' }));
    ...
    <section className="ype-field-drawer ype-size-guide-editor" data-ype-path="sizeGuide">
      <div className="ype-size-guide-head">
        <div>
          ...
          <p>
            {t({
              en: 'Fill in the size guide when available. Clear it when unavailable; the storefront will hide this section.',
              zh: '有尺码表就填写；没有可以清空，前台不会展示这个区块。',
            })}
          </p>
        </div>
        {hasContent ? (
      ...
      {!hasContent ? (
        <div className="ype-size-guide-empty">
          <strong>
            {t({ en: 'No size guide is configured for this product', zh: '当前产品未配置尺码表' })}
          </strong>
          <button type="button" onClick={applySampleGuide}>
            {t({ en: 'Add sample table', zh: '添加示例表格' })}
          </button>
        </div>
      ) : (
        <>
          <div className="ype-size-guide-meta">
            <label className="ype-field" data-ype-path="sizeGuide.title">
              <span className="ype-field-label">{t('标题')}</span>
              <input
                type="text"
                value={title}
                placeholder={t('尺码对应表')}
            ...
            <label className="ype-field" data-ype-path="sizeGuide.cornerLabel">
              <span className="ype-field-label">
                {t({ en: 'Top-left header', zh: '左上角表头' })}
              </span>
              <input
                type="text"
                value={cornerLabel}
                placeholder={t({ en: 'Height cm / Weight kg', zh: '身高 cm / 体重 kg' })}
          ...
          <div className="ype-size-guide-actions">
            <button type="button" onClick={addColumn}>
              {t({ en: 'Add weight column', zh: '添加体重列' })}
            </button>
            <button type="button" onClick={addRow} disabled={columns.length === 0}>
              {t({ en: 'Add height row', zh: '添加身高行' })}
            </button>
          </div>
          ...
          {columns.length === 0 ? (
            <p className="ype-field-help">
              {t({
                en: 'Add weight columns first, then fill in the size for each height range.',
                zh: '先添加体重列，再填写每个身高区间对应的尺码。',
              })}
            </p>
          ) : (
            <div className="ype-size-guide-table-wrap">
                      ...
                      {columns.map((column, columnIndex) => (
                        <td key={`${row.id ?? rowIndex}-${column.id ?? columnIndex}`}>
                          <input
                            aria-label={`${
                              textValue(row.label) ||
                              t({ en: `Row ${rowIndex + 1}`, zh: `第 ${rowIndex + 1} 行` })
                            } / ${
                              textValue(column.label) ||
                              t({ en: `Column ${columnIndex + 1}`, zh: `第 ${columnIndex + 1} 列` })
                            }`}
              ...
              {rows.length === 0 ? (
                <div className="ype-size-guide-no-rows">
                  {t({ en: 'No height rows yet.', zh: '还没有身高行。' })}
                </div>
              ) : null}
            </div>
          )}
