...
export function AdminLoginEnhancer({ accountEmail, usernameAlias }: AdminLoginEnhancerProps) {
  ...
  useEffect(() => {
    ...
    if (accountInput) {
      ...
      accountInput.placeholder = t('请输入登录账号');
      accountInput.spellcheck = false;
    }

    if (passwordInput) {
      passwordInput.placeholder = t('请输入密码');
    }
    ...
    if (
      ...
      button.type = 'button';
      button.textContent = t('显示');
      button.setAttribute('aria-label', t('显示密码'));

      button.addEventListener('click', () => {
        const isVisible = passwordInput.type === 'text';
        passwordInput.type = isVisible ? 'password' : 'text';
        button.textContent = isVisible ? t('显示') : t('隐藏');
        button.setAttribute('aria-label', isVisible ? t('显示密码') : t('隐藏密码'));
        passwordInput.focus();
      });
