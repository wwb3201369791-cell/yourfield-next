...
const previewDragActivationThreshold = 8;
const honorsAutoScrollResumeDelay = 1200;
const honorsAutoScrollSpeed = 0.24;
const honorsCarouselDuration = 28;
...
export function HonorsCarouselSection({
  ...
  const handlePreviewPointerDown = useCallback((event: PointerEvent<HTMLButtonElement>) => {
  ...
  const handlePreviewPointerMove = useCallback((event: PointerEvent<HTMLButtonElement>) => {
    const dragState = previewPointerStateRef.current;
    ...
    if (Math.hypot(deltaX, deltaY) > previewDragActivationThreshold) {
      dragState.didDrag = true;
    }
  }, []);
  const finishPreviewPointer = useCallback((event: PointerEvent<HTMLButtonElement>) => {
    const dragState = previewPointerStateRef.current;
  ...
  const handlePreviewClick = useCallback(
    (event: MouseEvent<HTMLButtonElement>, item: HonorImageItem) => {
      ...
      >
        <div className="honors-marquee">
          <Carousel
            ...
            autoScroll={{
              speed: honorsAutoScrollSpeed,
              startDelay: honorsAutoScrollResumeDelay,
            ...
            options={{
              ...
              containScroll: false,
              dragFree: true,
              duration: honorsCarouselDuration,
