...
describe('POST /api/revalidate', () => {
  ...
  it('revalidates product list and detail paths for a product publish payload', async () => {
    ...
    expect(revalidateTagMock).toHaveBeenCalledWith('cms:collection:products');
  });
  ...
  it('maps pages.home to the three localized homepage paths', async () => {
    ...
    expect(revalidateTagMock).toHaveBeenCalledWith('cms:collection:pages');
  });
  ...
  it('maps navigation and site settings changes to public key paths', async () => {
    ...
    expect(revalidateTagMock).toHaveBeenCalledWith('cms:global:navigation');
  });
  ...
  it('can resolve a slug from Payload when hook payload only has documentId', async () => {
    ...
    expect(revalidateTagMock).toHaveBeenCalledWith('cms:collection:news');
  });
});
