...
const removedNewsListColumns = new Set(['category']);
...
export function normalizeNewsCategoryPreferenceValue(value: unknown): unknown {
...
export async function up({ payload }: MigrateUpArgs): Promise<void> {
  ...
  for (const row of result.rows ?? []) {
    const normalizedValue = normalizeNewsCategoryPreferenceValue(row.value);
