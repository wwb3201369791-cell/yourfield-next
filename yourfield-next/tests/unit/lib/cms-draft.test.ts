...
describe('CMS news copy normalization', () => {
  it('replaces legacy seeded placeholder excerpts without overriding real CMS copy', async () => {
    const payload = createPayloadStub({
      news: [
        {
          content: {
...
describe('CMS product optional content normalization', () => {
  ...
  it('uses extracted product images when backend copy exists but images are blank', async () => {
    const payload = createPayloadStub({
      products: [
        {
          id: 'product-1',
    ...
    expect(items[0]).toMatchObject({
      ...
      image: '/images/products/extracted/live-line-shielding-suit/image-001.png',
      name: { zh: '后台保留的旧文案' },
    });
    expect(items[0]?.images.slice(0, 2)).toEqual([
      '/images/products/extracted/live-line-shielding-suit/image-001.png',
      '/images/products/extracted/live-line-shielding-suit/image-002.png',
    ]);
    expect(detail?.image).toBe('/images/products/extracted/live-line-shielding-suit/image-001.png');
  });
