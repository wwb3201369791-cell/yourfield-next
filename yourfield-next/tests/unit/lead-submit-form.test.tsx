...
async function waitForHydratedSubmit() {
  await waitFor(() => {
    expect(screen.getByRole<HTMLButtonElement>('button', { name: '发送咨询' }).disabled).toBe(false);
  });
}
...
describe('LeadSubmitForm', () => {
  ...
  it('requires at least a mobile number or email before submitting', async () => {
  ...
  it('saves the inquiry first and then opens an email draft for the user to send', async () => {
    ...
    fireEvent.change(screen.getByLabelText('姓名 *'), { target: { value: '测试客户' } });
    fireEvent.change(screen.getByLabelText('邮箱（手机二选一）'), {
      target: { value: 'lead@example.com' },
    });
    fireEvent.change(screen.getByLabelText('手机（邮箱二选一）'), {
