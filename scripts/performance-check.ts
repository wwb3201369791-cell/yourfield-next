...
function readPositiveInteger(value: ArgValue, fallback: number, label: string) {
...
export function resolvePerformanceCheckConfig(
  ...
  const profile = readProfile(
    readString(values.profile) ?? environment.PERF_CHECK_PROFILE,
    environment,
  );
  ...
  const paths = (
    ...
    readEnvironmentList(environment.PERF_CHECK_PATHS) ?? [...defaultPaths]
  ).map(normalizePath);
