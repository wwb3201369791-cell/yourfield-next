...
export const localizedFrom = (
  value: Partial<LocalizedString> | string,
  fallback: string,
): LocalizedString => {
  ...
  return localized(
    ...
    value.ru || value.en || value.zh || fallback,
  );
};
...
export const localizedTextRows = (
  values: readonly string[] | Partial<LocalizedString> | undefined,
) => {
  if (!values) {
    return [];
  }
...
export const localizedRichTextFromPlainText = (
  text: LocalizedString,
): Record<Locale, LexicalRichText> => ({
