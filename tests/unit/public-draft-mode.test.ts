...
async function loadPublicPages(isDraft: boolean) {
  ...
  vi.doMock('@/lib/i18n/route', () => ({
    resolveRouteLocale: vi.fn((locale: string) => (locale === 'en' || locale === 'ru' ? locale : 'zh')),
  }));
  ...
  const [home, products, productDetail, news, newsDetail, about] = await Promise.all([
    import('@/app/[locale]/(public)/page'),
    import('@/app/[locale]/(public)/products/page'),
    import('@/app/[locale]/(public)/products/[slug]/page'),
    import('@/app/[locale]/(public)/news/page'),
    import('@/app/[locale]/(public)/news/[slug]/page'),
    import('@/app/[locale]/(public)/about/page'),
  ]);
...
describe('public page Draft Mode integration', () => {
  it('passes draft=true into public CMS data fetches and marks preview metadata noindex', async () => {
    ...
    await modules.home.default({ params: { locale: 'zh' } });
    await modules.products.default({ params: { locale: 'zh' } });
    await modules.news.default({ params: { locale: 'zh' } });
    await modules.productDetail.default({
      params: { locale: 'zh', slug: 'draft-product' },
    });
    await modules.newsDetail.generateMetadata({
      params: { locale: 'zh', slug: 'draft-news' },
    });
    await modules.about.generateMetadata({ params: { locale: 'zh' } });
