import fieldSchema from 'payload/dist/fields/config/schema';
...
describe('admin i18n edit guide', () => {
  it.each([
    ...
    expect(field?.admin?.components?.Field).toBeTypeOf('function');
    expect(fieldSchema.validate(field, { abortEarly: false }).error).toBeUndefined();
