...
import type * as FormsSubmitRoute from '@/app/api/forms/submit/route';
...
const loadRoute = async ({
  ...
  const route: RouteModule = await import('@/app/api/forms/submit/route');
...
describe('POST /api/forms/submit', () => {
  ...
  it('rate limits spoofed x-forwarded-for submissions when proxy headers are not trusted', async () => {
    const { payload, route } = await loadRoute();

    const responses = await Promise.all(
      [1, 2, 3, 4].map((attempt) =>
        route.POST(
          formRequest(validSubmission({ email: `lead-${attempt}@example.com` }), {
